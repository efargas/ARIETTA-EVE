...
ft_void_t Clocks(ft_uint8_t clksize,ft_uint8_t options)
{
   ...
   do {
      ...
      for(i=-1; i<(per_f+2); i++) {
         ...
         timer(name);
         if(Tag==name+1) Ft_Gpu_CoCmd_BgColor(phost,0x4040a0);
         else
...
#endif
{
   ...
   printf("reg_touch_rz =0x%x\n", Ft_Gpu_Hal_Rd16(phost, REG_TOUCH_RZ));
   printf("reg_touch_rzthresh =0x%x\n", Ft_Gpu_Hal_Rd32(phost, REG_TOUCH_RZTHRESH));
   printf("reg_touch_tag_xy=0x%x\n",Ft_Gpu_Hal_Rd32(phost, REG_TOUCH_TAG_XY));
